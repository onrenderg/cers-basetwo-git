...
CREATE procedure [sec].[Mobile_finalsaveData]
...
Begin
	...
	begin try
		...
		select @pollDate=PollDate From sec.sec.electionmaster (nolock) where ElectionID = ( select max(electionid) from sec.sec.CandidatePersonalInfo  (nolock) where AUTO_ID=@AutoID )
